...
#ifdef DEBUG_SECONDSTAGE
    printf("[SECONDSTAGE] Starting second-stage address translation with GPA: 0x%lx, DID: %d, GV: %d\n", gpa, DID, GV);
#endif
...
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] iohgatp.MODE is Bare, no translation or protection\n");
#endif
...
#ifdef DEBUG_SECONDSTAGE
    printf("[SECONDSTAGE] Starting page table walk - i=%d, initial a=0x%lx\n", i, a);
#endif
...
step_2:
    ...
    if ( a & ~pa_mask ) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] Physical address access fault (a=0x%lx, pa_mask=0x%lx)\n", a, pa_mask);
#endif
        return GST_ACCESS_FAULT;
    }
    gpte->raw = 0;
#ifdef DEBUG_SECONDSTAGE
    printf("[SECONDSTAGE] Reading G-stage PTE from address 0x%lx, size: %d\n", (a | (vpn[i] * PTESIZE)), PTESIZE);
#endif
    status = read_memory(iommu, (a | (vpn[i] * PTESIZE)), PTESIZE, (char *)&gpte->raw,
                         rcid, mcid, PMA, endian);
    if ( status & ACCESS_FAULT ) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] G-stage PTE read access fault\n");
#endif
        return GST_ACCESS_FAULT;
    }
    if ( status & DATA_CORRUPTION) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] G-stage PTE data corruption detected\n");
#endif
        return GST_DATA_CORRUPTION;
    }
...
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] Invalid G-stage PTE detected - V:%d, R:%d, W:%d, PBMT:%d, reserved:%d, rsw60t59b:%d, Svpbmt:%d\n", 
               gpte->V, gpte->R, gpte->W, gpte->PBMT, gpte->reserved, gpte->rsw60t59b, iommu->reg_file.capabilities.Svpbmt);
#endif
    ...
    if ( gpte->R == 1 || gpte->X == 1 ) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] G-stage PTE is valid leaf (R=%d, X=%d), going to step_5\n", gpte->R, gpte->X);
#endif
        goto step_5;
    }
    ...
    if ( gpte->PBMT != 0 ) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] Non-leaf G-stage PTE has reserved PBMT bits set\n");
#endif
        return GST_PAGE_FAULT;
    }

    // For non-leaf PTEs, the D, A, and U bits are reserved for future standard use.
    if ( gpte->D != 0 || gpte->A != 0 || gpte->U != 0) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] Non-leaf G-stage PTE has reserved D/A/U bits set (D=%d, A=%d, U=%d)\n", gpte->D, gpte->A, gpte->U);
#endif
        return GST_PAGE_FAULT;
    }
    ...
    i = i - 1;
#ifdef DEBUG_SECONDSTAGE
    printf("[SECONDSTAGE] Non-leaf G-stage PTE - decrementing i to %d, PPN=0x%x, going to step_2\n", i, gpte->PPN);
#endif
    if ( i < 0 ) {
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] i < 0, returning GST_PAGE_FAULT\n");
#endif
        return GST_PAGE_FAULT;
    }
    a = gpte->PPN * PAGESIZE;
...
step_5:
    ...
    *gst_page_sz = PAGESIZE;
    ...
    count_events(iommu, PV, PID, PSCV, PSCID, DID, GV, GSCID, G_PT_WALKS);
    amo_gpte.raw = 0;
    status = read_memory_for_AMO(iommu, (a + (vpn[i] * PTESIZE)), PTESIZE,
    ...
    status = write_memory(iommu, (char *)&amo_gpte.raw, (a + (vpn[i] * PTESIZE)),
...
#ifdef DEBUG_SECONDSTAGE
        printf("[SECONDSTAGE] NAPOT G-stage PTE processing: updated PPN to 0x%x\n", gpte->PPN);
#endif
    }
...
#ifdef DEBUG_SECONDSTAGE
    printf("[SECONDSTAGE] Second-stage translation successful - PA: 0x%lx, gst_page_sz: %ld\n", *pa, *gst_page_sz);
#endif
    return 0;
}
