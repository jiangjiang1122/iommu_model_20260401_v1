...
process_commands(
    ...
    printf("[COMMANDS] Starting to process commands\n");
#endif
    uint8_t status, itag;
    int endian;
    uint64_t a;
...
#ifdef DEBUG_COMMANDS
    printf("[COMMANDS] Checking command queue status - cqon: %d, cqen: %d, cqmf: %d, cmd_ill: %d, cmd_to: %d, command_queue_stall_for_itag: %d, iofence_wait_pending_inv: %d\n", 
           iommu->reg_file.cqcsr.cqon, iommu->reg_file.cqcsr.cqen, iommu->reg_file.cqcsr.cqmf, 
           iommu->reg_file.cqcsr.cmd_ill, iommu->reg_file.cqcsr.cmd_to, 
           iommu->command_queue_stall_for_itag, iommu->iofence_wait_pending_inv);
#endif
...
#ifdef DEBUG_COMMANDS
        printf("[COMMANDS] Command queue not ready, returning\n");
#endif
        return;
    }
...
#ifdef DEBUG_COMMANDS
        printf("[COMMANDS] Command queue is empty (cqh.index == cqt.index: %d), returning\n", iommu->reg_file.cqh.index);
#endif
        return;
    }
...
#ifdef DEBUG_COMMANDS
    printf("[COMMANDS] Reading command from address 0x%lx, CQ_ENTRY_SZ: %d\n", a, CQ_ENTRY_SZ);
#endif
    status = (a  & ~pa_mask) ?
             ACCESS_FAULT :
             read_memory(iommu, a, CQ_ENTRY_SZ, (char *)&command,
                         iommu->reg_file.iommu_qosid.rcid, iommu->reg_file.iommu_qosid.mcid,
                         PMA, endian);
    if ( status != 0 ) {
#ifdef DEBUG_COMMANDS
        printf("[COMMANDS] Command read failed with status: %d\n", status);
#endif
        // If command-queue access leads to a memory fault then the
...
#ifdef DEBUG_COMMANDS
    printf("[COMMANDS] Processing command with opcode: %d, func3: %d\n", command.any.opcode, command.any.func3);
#endif
...
#ifdef DEBUG_COMMANDS
    printf("[COMMANDS] Command processed successfully, advancing CQH to index: %d\n", iommu->reg_file.cqh.index);
#endif
    return;
...
command_illegal:
    ...
    // command processing software should clear this bit by writing 1
#ifdef DEBUG_COMMANDS
    printf("[COMMANDS] Illegal command detected, setting cmd_ill bit\n");
#endif
...
do_ats_msg(
    ...
    msg.PRIV    = 0;
    msg.EXEC_REQ= 0;
    send_msg_iommu_to_hb(iommu, &msg);
    return;
}
uint8_t
do_iofence_c(
    ...
    if ( AV == 1 ) {
        status = (ADDR & ~pa_mask) ?
                 ACCESS_FAULT :
                 write_memory(iommu, (char *)&DATA, ADDR, 4, iommu->reg_file.iommu_qosid.rcid,
