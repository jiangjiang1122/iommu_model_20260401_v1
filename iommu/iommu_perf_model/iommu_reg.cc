...
static uint8_t offset_to_size[8192];
...
is_access_valid(
    ...
    if ( (num_bytes != 4 && num_bytes != 8) ||       // only 4B & 8B registers in IOMMU
         (offset >= 8192) ||                         // Offset must be <= 4095
...
read_register(
    ...
    if(offset <= IOMMU_INTERNAL_REG_OFFSET)
    {
        value = ( num_bytes == 4 ) ? iommu->reg_file.regs4[offset/4] :
                                    iommu->reg_file.regs8[offset/8];
    }
    else
    {
        value = ( num_bytes == 4 ) ? iommu->internal_reg_file.regs4[offset/4] :
                                    iommu->internal_reg_file.regs8[offset/8];
    }
    
    // 添加调试信息，特别关注DDTP寄存器的读取
    if (offset == DDTP_OFFSET) {
        printf("[DEBUG] Reading DDTP register at offset 0x%x, value: 0x%lx\n", offset, value);
        ddtp_t ddtp_val;
        ddtp_val.raw = value;
        printf("[DEBUG] DDTP register details - iommu_mode: %d, ppn: 0x%lx, busy: %d\n", 
               ddtp_val.iommu_mode, ddtp_val.ppn, ddtp_val.busy);
    }
...
write_register(
    ...
    {
        switch (offset) {
        case IOMMU_RC0_BUS_RANGE_OFFSET:
            iommu->internal_reg_file.rc_bus_range[0] = data4;
            return;
        case IOMMU_RC1_BUS_RANGE_OFFSET:
            iommu->internal_reg_file.rc_bus_range[1] = data4;
            return;
        case IOMMU_RC2_BUS_RANGE_OFFSET:
            iommu->internal_reg_file.rc_bus_range[2] = data4;
            return;
        case IOMMU_RC3_BUS_RANGE_OFFSET:
            iommu->internal_reg_file.rc_bus_range[3] = data4;
            return;
        case IOMMU_RC0_SEGMENT_ID_OFFSET:
            iommu->internal_reg_file.rc_segment_id[0] = data4;
            return;
        case IOMMU_RC1_SEGMENT_ID_OFFSET:
            iommu->internal_reg_file.rc_segment_id[1] = data4;
            return;
        case IOMMU_RC2_SEGMENT_ID_OFFSET:
            iommu->internal_reg_file.rc_segment_id[2] = data4;
            return;
        case IOMMU_RC3_SEGMENT_ID_OFFSET:
            iommu->internal_reg_file.rc_segment_id[3] = data4;
            return;
        case IOMMU_WARL_HEAD_POINT_OFFSET:
            iommu->internal_reg_file.wral_head_point = data4;
            return;
        case IOMMU_WARL_TAIL_POINT_OFFSET:
            iommu->internal_reg_file.wral_tail_point = data4;
            return;
        }
        return;
    }
    ...
    // 添加调试信息，特别关注DDTP寄存器的写入
    if (offset == DDTP_OFFSET) {
        printf("[DEBUG] Writing to DDTP register at offset 0x%x, data: 0x%lx\n", offset, data8);
        ddtp_t ddtp_new_val;
        ddtp_new_val.raw = data8;
        printf("[DEBUG] Writing DDTP register details - iommu_mode: %d, ppn: 0x%lx, busy: %d\n", 
               ddtp_new_val.iommu_mode, ddtp_new_val.ppn, ddtp_new_val.busy);
    }
