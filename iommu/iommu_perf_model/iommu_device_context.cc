...
locate_device_context(
    ...
    printf("[LOCATE_DC] Starting device context lookup for device_id: 0x%x\n", device_id);
    printf("[LOCATE_DC] DDTP raw value: 0x%lx, mode: %d, ppn: 0x%lx\n", 
           iommu->reg_file.ddtp.raw, iommu->reg_file.ddtp.iommu_mode, iommu->reg_file.ddtp.ppn);
    printf("[LOCATE_DC] Capabilities PAS: %d, pa_mask: 0x%lx\n", 
           iommu->reg_file.capabilities.pas, pa_mask);
#endif
...
#ifdef DEBUG_TRANSLATION
    printf("[LOCATE_DC] Step 3: Calculating address for DDT lookup\n");
    printf("[LOCATE_DC] a = 0x%lx, DDI[%d] = %d, a + (DDI[i] * 8) = 0x%lx\n", a, i, DDI[i], a + (DDI[i] * 8));
    printf("[LOCATE_DC] pa_mask = 0x%lx, a & ~pa_mask = 0x%lx\n", pa_mask, a & ~pa_mask);
#endif

    status = (a  & ~pa_mask) ?
             ACCESS_FAULT :
             read_memory(iommu, (a + (DDI[i] * 8)), 8, (char *)&ddte.raw,
                         iommu->reg_file.iommu_qosid.rcid,
                         iommu->reg_file.iommu_qosid.mcid, PMA, endian);
#ifdef DEBUG_TRANSLATION
    printf("[LOCATE_DC] Read memory status: %d\n", status);
#endif
...
#ifdef DEBUG_TRANSLATION
    printf("[LOCATE_DC] Step 7: Updating indices - i=%d, ddte.PPN=0x%lx, new a=0x%lx\n", i, ddte.PPN, ddte.PPN * PAGESIZE);
#endif
    i = i - 1;
    a = ddte.PPN * PAGESIZE;
    goto step_2;
...
step_8:
...
#ifdef DEBUG_TRANSLATION
    printf("[LOCATE_DC] Step 9: Checking DC validity - DC->tc.V = %d\n", DC->tc.V);
#endif
    if ( DC->tc.V == 0 ) {
        *cause = 258;     // DDT entry not valid
        return 1;
    ...
    if ( do_device_context_configuration_checks(iommu, DC) ) {
        *cause = 259;     // DDT entry misconfigured
#ifdef DEBUG_TRANSLATION
        printf("[LOCATE_DC] Device context configuration check failed\n");
#endif
        return 1;
    }
    //12. The device-context has been successfully located and may be cached.
#ifdef DEBUG_TRANSLATION
    printf("[LOCATE_DC] Successfully located device context for device_id: 0x%x\n", device_id);
#endif
...
do_device_context_configuration_checks(
    ...
    if ( ((iommu->reg_file.capabilities.msi_flat == 1) && (DC->reserved != 0)) ||
         ((iommu->reg_file.capabilities.msi_flat == 1) && (DC->msiptp.reserved0 != 0)) ||
