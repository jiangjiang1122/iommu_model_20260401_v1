...
msi_address_translation(
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Starting MSI address translation with gpa: 0x%lx, is_exec: %d\n", gpa, is_exec);
#endif
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Initial values - pa_mask: 0x%lx, endian: %s\n", pa_mask, endian == BIG_ENDIAN ? "BIG_ENDIAN" : "LITTLE_ENDIAN");
#endif
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Calculated MGPAW: %lu, MGPAW_MASK: 0x%lx\n", mgpaw, mgpaw_mask);
#endif
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 1: A set to GPA 0x%lx\n", A);
#endif
    ...
    *is_msi = (((A >> 12) & ~DC->msi_addr_mask.mask & mgpaw_mask) ==
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 3: Checking for MSI - A>>12: 0x%lx, mask: 0x%lx, pattern: 0x%lx, is_msi: %d\n", 
           A >> 12, (~DC->msi_addr_mask.mask & mgpaw_mask), (DC->msi_addr_pattern.pattern & ~DC->msi_addr_mask.mask & mgpaw_mask), *is_msi);
#endif
    ...
    if ( *is_msi == 0 ) {
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] Step 4: Address is not MSI, returning 0\n");
#endif
        return 0;
    }
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 5: I extracted as 0x%lx from A>>12: 0x%lx with mask: 0x%lx\n", I, (A >> 12), (DC->msi_addr_mask.mask & mgpaw_mask));
#endif
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 6: m set to PPN*PAGESIZE: 0x%lx\n", m);
#endif
    ...
    status = ((m | (I * 16)) & ~pa_mask) ?
             ACCESS_FAULT :
             read_memory(iommu, (m | (I * 16)), 16, (char *)&msipte.raw, rcid, mcid,
             PMA, endian);

#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 7: Reading MSI PTE from address 0x%lx, status: %d\n", (m | (I * 16)), status);
#endif

    if ( status & ACCESS_FAULT ) {
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI PTE load access fault, cause: 261\n");
#endif
...
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI PT data corruption detected, cause: 270\n");
#endif
    ...
    if ( msipte.V == 0 ) {
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI PTE not valid, cause: 262\n");
#endif
    ...
    if ( msipte.C == 1 ) {
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI PTE misconfigured (C==1), cause: 263\n");
#endif
        *cause = 263;
        return 1;
    }
...
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI PTE misconfigured (M=%d), cause: 263\n", msipte.M);
#endif
        *cause = 263;
        return 1;
    }
    ...
    if ( msipte.M == 3 ) {
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] Step 13: MSI PTE in Translate/RW mode (M=3)\n");
#endif
        if ( msipte.translate_rw.reserved != 0 || msipte.translate_rw.reserved0 != 0 ) {
#ifdef DEBUG_MSITRANS
            printf("[MSI_TRANSLATION] MSI PTE misconfigured (reserved bits set), cause: 263\n");
#endif
            *cause = 263;
            return 1;
        }
...
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] Step 13b: Translated address computed - pa: 0x%lx, page_sz: %ld\n", *pa, *page_sz);
#endif
        goto step_15;
    }
...
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI MRIF not supported, cause: 263\n");
#endif
        *cause = 263;
        return 1;
    }
...
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] MSI PTE MRIF mode has reserved bits set, cause: 263\n");
#endif
        *cause = 263;
        return 1;
    }
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 14d: Destination MRIF address set to 0x%lx\n", *dest_mrif_addr);
#endif
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 14e: Destination PA set to 0x%lx\n", *pa);
#endif
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] Step 14f: MRIF NID set to %d\n", *mrif_nid);
#endif
...
#ifdef DEBUG_MSITRANS
        printf("[MSI_TRANSLATION] Instruction access fault (is_exec=%d, check_access_perms=%d), cause: 1\n", is_exec, check_access_perms);
#endif
        *cause = 1;
        return 1;
    }
...
#ifdef DEBUG_MSITRANS
    printf("[MSI_TRANSLATION] MSI address translation completed successfully, returning 0\n");
#endif
    return 0;
}
