...
- Root Port测试：通过RP模块向IOMMU发起设备请求，验证地址转换（单级/两级）、故障记录与响应一致性。
...
participant SLINK as "SLINK_Module"
participant DDR as "DDR_Module"
RP->>IOMMU : 发送AT请求(nb_transport_fw)
IOMMU->>SLINK : 页表访问请求
...
```mermaid
flowchart TD
Start(["接收请求"]) --> CheckCmd["判断命令类型(读/写)"]
...
## 结论
本测试框架通过RP/DDR/SLINK/PCIENOC四大模块协同，覆盖了RISC-V IOMMU的地址转换、NoC路径与内存访问等关键路径。借助多场景测试线程与统计输出，能够有效评估不同页表模式与并发条件下的性能与稳定性，并为回归测试与性能基准提供基础。
