...
- [test_sequential_final.txt](file://test_sequential_final.txt)
- [test_walker_parallel.txt](file://test_walker_parallel.txt)
...
## 更新摘要
**变更内容**
- 新增SLINK NoC延迟性能测试框架，包括完整的延迟阶梯测试、临界点精确定位和验证机制
- 添加多个测试脚本用于自动化性能分析和瓶颈检测
- 扩展测试数据文件，提供详细的性能基准和验证输出
- 增强SLINK模块的性能分析能力，支持动态延迟参数调整
...
**新增**：SLINK NoC延迟性能测试框架提供了完整的延迟阶梯测试、临界点精确定位和验证机制，包括test_slink_verify.sh、test_precise.sh等多个自动化测试脚本。
...
- **新增**：test_slink_verify.sh、test_precise.sh、test_slink_latency.sh、test_slink_critical.sh等SLINK性能测试脚本。
...
- **延迟阶梯测试**
  ...
  - test_slink_critical.sh：精确验证临界点，从150ns开始逐步增加到瓶颈点。
- **性能分析与验证**
...
## 结论
本测试框架通过RP/DDR/SLINK/PCIENOC四大模块协同，覆盖了RISC-V IOMMU的地址转换、NoC路径与内存访问等关键路径。**更新**：payload大小从16字节增加到512字节，使测试更贴近真实DMA流量模式，提高了测试的有效性和代表性。**新增**：SLINK NoC延迟性能测试框架提供了完整的自动化测试解决方案，包括延迟阶梯测试、临界点精确定位和性能验证机制。

借助多场景测试线程与统计输出，能够有效评估不同页表模式与并发条件下的性能与稳定性，并为回归测试与性能基准提供基础。SLINK测试框架特别适用于NoC路径的性能分析和瓶颈定位，为系统优化提供了重要的数据支撑。
...
- **新增**：SLINK性能测试支持持续集成，可自动检测性能回归和瓶颈变化。
...
### SLINK性能测试详细说明（新增）
- **test_slink_verify.sh**
  - 功能：验证PTW延时和IOPS，自动调整SLINK延迟参数并提取性能指标。
  - 测试范围：150ns到10000ns的延迟阶梯。
  - 输出：每个测试点的IOMMU IOPS、PTW IOPS、PTW平均执行延迟、PTW DDR访问次数、稳态IOPS。
  - 验证机制：基于理论计算验证PTW延时是否符合预期。
- **test_precise.sh**
  - 功能：精确定位临界点，使用基于DDR=64的新理论进行测试。
  - 测试范围：500ns到1200ns的精细延迟范围。
  - 输出：详细的性能统计和瓶颈检测结果。
- **test_slink_latency.sh**
  - 功能：基础延迟阶梯测试，适合快速性能评估。
  - 测试范围：150ns到20000ns的广泛延迟范围。
- **test_slink_critical.sh**
  - 功能：精确验证临界点，逐步增加延迟直到性能下降。
  - 测试范围：150ns到8000ns的逐步增加测试。
  - 输出：性能下降检测点和临界点识别。
