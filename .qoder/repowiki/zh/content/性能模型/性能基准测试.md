...
## 更新摘要
**变更内容**   
- 新增Scene 10构建目标，复用Scene 7的所有配置宏以保持构建系统一致性
- 为失效测试场景提供专用编译目标，支持独立的构建和测试流程
- 增强构建系统的可扩展性，支持更多测试场景的灵活配置
- 优化Makefile结构，统一不同场景的编译参数管理
...
**更新** 本次更新重点反映了构建系统的扩展，新增了Scene 10构建目标，该目标复用Scene 7的所有配置宏以保持一致性，为失效测试场景提供了专用的编译目标。这一改进增强了构建系统的可扩展性和维护性，使得新的测试场景能够更便捷地集成到现有的测试框架中。
...
SCENE9["Scenario 9测试<br/>250M IOPS + 128GB/s带宽验证"]
SCENE10["新增Scene 10测试<br/>失效测试场景<br/>复用Scene 7配置宏"]
END
subgraph "外部接口"
...
- **专门测试场景**：多个独立的测试场景文件分别专注于不同的访问模式和地址翻译场景，包括新增的Scene 10失效测试场景，该场景复用Scene 7的配置宏以保持构建系统的一致性，支持顺序与随机访问、单级与双级地址翻译、不同页表规模与请求量，具备稳态IOPS采样窗口与结果校验。
...
**更新** 核心组件现在包含了新增的Scene 10失效测试场景，该场景通过复用Scene 7的配置宏，确保了构建系统的一致性和可维护性。新增的Scene 10测试场景为失效场景的专门测试提供了支持，进一步完善了测试覆盖范围。
...
## 架构总览
IOMMU性能基准测试围绕"请求注入—解析—缓存查询—页表遍历—转发/故障"闭环展开，性能收集器在关键节点记录事件与延迟，统计采集器汇总形成统一报告；多个专门的测试场景通过不同访问模式与地址翻译配置构造多样化场景，特别是新增的Scene 10失效测试场景通过复用Scene 7的配置宏，实现了构建系统的一致性和可维护性，**新增的PTW参数化支持**使得场景5、6、7、10可以进行灵活的并发级别和带宽配置测试。
...
participant SCENE9 as "Scenario 9测试<br/>250M IOPS + 128GB/s"
participant SCENE10 as "新增Scene 10测试<br/>失效测试场景<br/>复用Scene 7配置"
...
SCENE10->>IOMMU : 注入失效测试请求<br/>复用Scene 7配置
IOMMU->>COL : 解析后路由到收集器
COL->>DC : 查询DC缓存
COL->>PC : 查询PC缓存
alt DC命中
...
- [test_rp_seq128k_two_stage_thread.cc:260-377](file://rp/test_rp_seq128k_two_stage_thread.cc#L260-377)
...
#### 场景10：新增失效测试场景
- **配置**：复用Scene 7的所有配置宏，确保构建系统的一致性
- **特点**：专门为失效测试场景设计的专用编译目标，支持失效条件的全面测试
- **地址翻译**：继承Scene 7的地址翻译配置，保证测试环境的一致性
- **构建优化**：通过复用配置宏，减少代码重复和维护成本
- **应用场景**：失效场景的专门测试和验证，提高测试覆盖率
...
participant TEST9 as "Scenario 9测试<br/>250M IOPS"
participant TEST10 as "新增Scene 10测试<br/>失效测试场景"
participant IOMMU as "IOMMU顶层"
...
TEST10->>IOMMU : 配置失效测试参数<br/>复用Scene 7配置
...
- **场景兼容性**：确保场景5、6、7、10都能正确使用SCENE7_PTW参数进行配置

**更新** 新增PTW参数化系统，通过Makefile中的SCENE7_PTW参数，实现了场景5、6、7、10的灵活测试能力，支持不同PTW并发级别和带宽配置。
...
- **PTW参数化依赖**：Makefile中的SCENE7_PTW参数影响场景5、6、7、10的编译和运行配置。
...
MAIN --> SCENE9["Scenario 9测试<br/>250M IOPS + 128GB/s"]
MAIN --> SCENE10["新增Scene 10测试<br/>失效测试场景<br/>复用Scene 7配置"]
...
- **Scene 9性能突破**：新增的Scene 9测试场景代表了当前IOMMU性能测试的最高水平，通过优化的并发控制和内存管理机制，实现了250M IOPS和128GB/s带宽的极致性能表现。
- **Scene 10构建优化**：新增的Scene 10测试场景通过复用Scene 7的配置宏，确保了构建系统的一致性和可维护性，为失效测试场景提供了专门的编译目标。

**更新** 新增的Scene 10测试场景通过复用Scene 7的配置宏，确保了构建系统的一致性和可维护性，为失效测试场景提供了专门的编译目标，进一步完善了测试覆盖范围。
...
- **测试场景特定问题**：
  ...
  - **Scene 9测试**：验证250M IOPS和128GB/s带宽目标的达成情况，检查极端负载下的系统稳定性
  - **Scene 10测试**：验证失效测试场景的正确性，检查复用Scene 7配置宏的效果和构建系统的一致性

**更新** 新增了Scene 10测试场景的故障排查要点，需要特别关注失效测试场景的正确性和构建系统的一致性。
...
## 结论
本基准测试体系以SystemC仿真为核心，通过多个专门的测试场景文件实现了从请求注入、缓存查询、页表遍历到结果验证的全链路性能评估。测试场景重构提供了更精细的测试覆盖，能够分别验证顺序访问、随机访问、双级地址翻译、极端高性能负载和PTW参数化等不同场景下的性能表现。新增的Scenario 6高性能测试场景特别支持128GB/s吞吐量、512并发操作和PTW并发级别5，通过编译时宏注入的参数优化，实现了250M IOPS的100%效率，充分验证了IOMMU在高负载下的性能极限和优化潜力。**新增的PTW参数化支持**使得场景5、6、7、10可以进行灵活的并发级别和带宽配置测试，为不同应用场景提供了更精确的性能评估能力。**新增的Scenario 9测试场景**代表了当前IOMMU性能测试的最高水平，通过优化的并发控制和内存管理机制，实现了250M IOPS和128GB/s带宽的极致性能表现。**新增的Scene 10测试场景**通过复用Scene 7的配置宏，确保了构建系统的一致性和可维护性，为失效测试场景提供了专门的编译目标，进一步完善了测试覆盖范围。

**更新** 重构后的测试体系通过多个专门的测试场景文件，特别是新增的Scenario 6高性能测试场景、Scenario 9极致性能测试场景、Scene 10失效测试场景、PTW参数化支持和增强的参数系统，提供了更精确的测试覆盖和更好的可维护性，每个场景都针对特定的性能特征和使用模式进行了优化，为IOMMU的性能基准测试提供了全面的支持。
...
- **测试场景选择**：
  ...
  - `make TEST=scenario9_extreme`：Scenario 9测试（250M IOPS + 128GB/s带宽）
  - `make TEST=scene10_failure`：新增Scene 10失效测试（复用Scene 7配置）
  ...
  - **新增PTW参数化测试**：`make TEST=scene10_ptw_param SCENE7_PTW=<value>`：Scene 10 PTW参数化测试

**更新** 新增了Scene 10失效测试场景和PTW参数化测试的编译选项，支持通过编译时宏注入和SCENE7_PTW参数进行配置。
...
- **场景10：新增失效测试场景**
  - 访问模式：失效条件测试（复用Scene 7配置宏）
  - 地址翻译：继承Scene 7的地址翻译配置，保证测试环境的一致性
  - 性能目标：验证失效场景的处理能力和系统稳定性
  - 参数配置：通过复用Scene 7的配置宏，确保构建系统的一致性
  - 特点：为失效场景的专门测试提供支持，提高测试覆盖率

**更新** 新增了Scene 10失效测试场景的详细配置说明，该场景通过复用Scene 7的配置宏，确保了构建系统的一致性和可维护性。
...
- **测试场景特定流程**：
  ...
  - **Scene 9测试**：验证250M IOPS和128GB/s带宽目标的达成情况，监控极端负载下的系统稳定性
  - **Scene 10测试**：验证失效测试场景的正确性，检查复用Scene 7配置宏的效果和构建系统的一致性

**更新** 新增了Scene 10测试场景的特定流程和监控要点，需要特别关注失效测试场景的正确性和构建系统的一致性。
...
- **关键指标**
  ...
  - 250M IOPS达成率、128GB/s带宽验证（Scene 9测试）
  - 失效场景处理成功率、构建系统一致性验证（Scene 10测试）
- **统计摘要**
  ...
  - Scene 9性能指标详情（250M IOPS和128GB/s带宽验证）
  - Scene 10构建配置详情（复用Scene 7配置宏的验证）
- **结论**
  - 功能验证通过与否、性能指标评述、改进建议、性能极限评估

**更新** 新增了Scene 10测试场景相关的指标和配置记录要求，包括失效场景处理成功率和构建系统一致性验证。
...
- **PTW参数化对比**：针对场景5、6、7、10，对比不同SCENE7_PTW参数配置下的性能表现，验证PTW并发级别和带宽配置的效果。
- **Scene 9性能基准对比**：针对新增的Scene 9测试场景，建立250M IOPS和128GB/s带宽的性能基准，用于后续版本的性能回归测试。
- **Scene 10构建一致性对比**：验证Scene 10复用Scene 7配置宏的效果，确保构建系统的一致性和可维护性。

**更新** 新增了Scene 10构建一致性对比的实验设计，验证Scene 10复用Scene 7配置宏的效果，确保构建系统的一致性和可维护性。
...
**更新** 新增了Scene 10测试场景的统计分析和结果验证技术要求，需要特别关注失效测试场景的正确性和构建系统的一致性。
...
- **案例7：Scene 10失效测试案例**
  - 关键统计：失效场景处理成功率、构建系统一致性验证、复用Scene 7配置宏的效果
  - 性能验证：验证失效测试场景的正确性和构建系统的一致性

**更新** 新增了Scene 10失效测试案例，该案例验证了失效测试场景的正确性和构建系统的一致性。
...
### J. Scene 10失效测试场景详解
- **测试目标**：为失效测试场景提供专用的编译目标，确保构建系统的一致性
- **核心特性**：
  - 复用Scene 7的所有配置宏，保持构建系统的一致性
  - 专门的失效测试场景编译目标，支持独立的构建和测试流程
  - 继承Scene 7的地址翻译配置，保证测试环境的一致性
  - 增强的构建系统可扩展性，支持更多测试场景的灵活配置
- **性能指标**：
  - 失效场景处理成功率：验证失效条件的正确处理
  - 构建系统一致性：确保复用配置宏的效果
  - 测试覆盖率：提高失效场景的测试覆盖
  - 维护性：减少代码重复和维护成本
- **应用场景**：
...
**更新** 新增了Scene 10失效测试场景的详细说明，该场景通过复用Scene 7的配置宏，确保了构建系统的一致性和可维护性，为失效测试场景提供了专门的编译目标。
