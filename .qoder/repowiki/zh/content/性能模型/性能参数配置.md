...
## 更新摘要
**变更内容**
- 增强了IOMMU性能参数系统，在iommu_perf_params.hh中添加了Makefile TEST_CFG_*宏支持，用于构建时配置端口宽度（1024位AXI）、全局未完成请求数、PT去重缓冲区大小和PTW未完成任务
- 更新了default_config.json中的DC/PC read_set_latency_cycles从5ns减少到4ns，优化缓存读取延迟
- 新增编译时参数配置功能，支持通过Makefile标志动态控制关键性能参数，无需修改源代码即可调整系统行为
...
**更新** 增强了IOMMU性能参数系统的构建时配置能力，通过Makefile的TEST_CFG_*宏支持动态配置端口宽度（1024位AXI）、全局未完成请求数、PT去重缓冲区大小和PTW未完成任务。更新了缓存读取延迟参数，将DC/PC read_set_latency_cycles从5ns优化到4ns，提升缓存访问性能。新增的编译时参数配置功能提供了更灵活的参数调优能力，无需修改源代码即可调整关键性能参数。
...
- [test_ddr.hh:17-65](file://ddr/test_ddr.hh#L17-L65)
...
## 架构总览
IOMMU性能参数配置系统通过"参数定义文件 + 缓存配置文件 + 解析器线程 + 任务上下文 + DDR内存模块 + 编译时参数配置 + Walker Cache"的组合实现：
...
- **缓存延迟优化**：DC/PC read_set_latency_cycles从5ns优化到4ns，提升了缓存读取性能，减少了整体访问延迟。
...
## 结论
IOMMU性能参数配置系统通过参数定义文件与缓存配置文件实现对仿真行为的精细控制。结合解析器线程与任务上下文，系统能够在不同工作负载下实现高并发、低延迟与大容量的平衡。**最新的增强功能包括：通过Makefile的TEST_CFG_*宏支持构建时配置端口宽度（1024位AXI）、全局未完成请求数、PT去重缓冲区大小和PTW未完成任务，提供了更灵活的参数调优能力。同时，DC/PC read_set_latency_cycles从5ns优化到4ns，进一步提升了缓存访问性能。** 新增的Walker Cache功能提供了三级页表缓存，可在重复访问场景下显著提升性能。建议以实际测试数据为依据，逐步调优FIFO深度、缓存规模、并发限制与去重/预取策略，并利用新的编译时参数配置功能快速验证不同配置组合的性能表现，建立稳定的评估方法与自动化验证流程。
...
- **编译时参数调优**：利用编译时参数快速测试不同的端口宽度（64、128、256、512、1024位）、全局未完成请求数（16、32、64、128、256）、PT去重缓冲区大小（256、512、1024、2048）和PTW未完成任务数（8、16、32、64），通过对比测试结果选择最优参数组合。
...
- **编译时参数优化**：针对不同工作负载场景，通过编译时参数快速调整端口宽度、全局未完成请求数、PT去重缓冲区大小和PTW未完成任务数，评估不同配置对性能的影响。
...
- **编译时参数评估**：通过对比不同编译时参数配置下的测试结果，量化参数变更对性能的影响，特别是端口宽度、全局未完成请求数、PT去重缓冲区大小和PTW未完成任务数的影响。
