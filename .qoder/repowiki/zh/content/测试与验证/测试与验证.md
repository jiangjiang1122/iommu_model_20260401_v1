...
## 更新摘要
**变更内容**   
- 增强了测试框架，新增Scenario 7覆盖高带宽随机访问模式下的两阶段翻译场景
- 更新了Makefile配置以支持新的测试场景和条件编译支持
- 在test_rp_rand4k_two_stage_thread.cc中添加了可配置的PAGES_NEEDED参数支持
- 改进了两阶段地址转换场景的测试覆盖范围和性能验证能力
...
**新增功能** 测试框架现在包含Scenario 7，专门针对高带宽随机访问模式下的两阶段地址转换进行性能验证，支持可配置的PAGES_NEEDED参数以适应不同的测试需求。
...
- **测试线程与场景**：
  ...
  - **新增Scenario 7**：支持高带宽随机访问模式下的两阶段地址转换测试
...
**重要更新** 测试框架简化了线程管理机制，移除了复杂的多线程协调逻辑，现在采用单线程实现，显著降低了测试系统的复杂度和维护成本。新增两阶段随机预取测试场景，专门验证复杂地址转换场景下的预取效果。**虚拟阶段页表测试基础设施得到改进，包括128K顺序访问测试用例的VS页表映射范围计算修正和IOVA_RANGE参数调整**。
...
- 测试场景配置：
  ...
  - **新增PAGES_NEEDED参数**：支持可配置的页面数量以满足不同带宽测试需求
- 关键验证点：
  - 随机访问场景下的功能正确性验证
...
**更新** 测试框架经过重构，现在采用明确的测试场景分类系统和目标配置机制。新增rand4k_twostage测试场景，专门验证两阶段随机预取功能。**虚拟阶段页表测试基础设施的改进确保了更准确的VS页表映射范围计算**。
...
- **TEST宏系统**：通过TEST宏选择不同的测试场景，包括sv48_bare、seq128k_singlestage、seq128k_twostage、**rand4k_twostage**等。
...
**更新** Makefile经过重构，现在采用明确的测试场景分类和目标配置系统。新增rand4k_twostage测试场景，配置两阶段随机预取测试。**虚拟阶段页表测试基础设施的改进提升了整体测试的可靠性**。

**章节来源**
- [Makefile:1-181](file://Makefile#L1-181)
...
**重要更新** 测试线程系统经过重构，移除了复杂的多线程协调机制，现在采用简化的单线程实现，显著降低了测试系统的复杂度。**虚拟阶段页表测试基础设施的改进增强了页表映射的准确性和调试能力**。
...
**更新** 新增了专门的性能分析工具，提供更详细的预取时序分析和统计报告，支持两阶段随机预取测试场景的性能评估。**虚拟阶段页表测试基础设施的改进使得性能分析更加准确可靠**。
...
**更新** 增强了测试报告模板，支持更详细的测试结果展示和性能分析，特别适合两阶段随机预取测试场景的结果报告。**虚拟阶段页表测试基础设施的改进需要在测试报告中体现其带来的可靠性提升**。
...
**更新** 测试框架重构后，不同测试场景的性能特征更加清晰，单阶段和双阶段测试在预取策略和性能表现上有明确区分。**虚拟阶段页表测试基础设施的改进特别提升了128K顺序访问测试场景的性能稳定性和调试能力**。新增的两阶段随机预取测试场景专门验证复杂地址转换场景下的预取效果。
...
**重要更新** 测试框架简化后，故障排查现在包括测试场景选择和目标配置相关的故障排查，同时减少了多线程相关的复杂问题。**新增虚拟阶段页表测试基础设施相关的故障排查指南，帮助定位VS页表映射范围计算和IOVA_RANGE参数配置问题**。新增两阶段随机预取测试场景的故障排查指南和Scenario 7相关问题的排查方法。
...
- [test_precise.sh:10-50](file://test_precise.sh#L10-L50)
- [Makefile:45-66](file://Makefile#L45-L66)
...
**重要更新** 测试框架的重大简化显著提升了系统的稳定性和可维护性，移除的实验性多线程机制减少了400+行无用代码，简化了测试协调机制，使测试系统更加专注于核心功能验证。**虚拟阶段页表测试基础设施的改进包括VS页表映射范围计算修正、IOVA_RANGE参数调整和TOTAL_IOVA_PAGES计算公式优化，显著提升了测试的可靠性和调试能力**。新增的两阶段随机预取测试场景为复杂地址转换场景的性能验证提供了重要支撑。**新增的Scenario 7和高带宽随机访问模式测试进一步扩展了测试框架的覆盖范围**。
...
**更新** 测试框架重构后，测试场景编写现在遵循明确的分类原则和命名约定。**虚拟阶段页表测试基础设施的改进要求在编写两阶段测试用例时特别注意VS页表映射范围计算的准确性**。新增两阶段随机预取测试场景，专门验证复杂地址转换场景下的预取效果。
...
**更新** 测试脚本使用指南现在包括重构后的测试场景分类和目标配置系统。**虚拟阶段页表测试基础设施的改进要求在运行两阶段测试时特别注意参数配置的准确性**。新增两阶段随机预取测试场景的使用指南和Scenario 7测试的使用方法。
...
**更新** 测试结果分析现在包括测试场景分类和大规模测试验证的要求。**虚拟阶段页表测试基础设施的改进要求在验证两阶段地址转换时特别关注VS页表映射范围的准确性**。新增两阶段随机预取测试场景的验证标准和Scenario 7的性能验证要求。
...
**更新** 测试环境搭建现在包括测试场景分类和目标配置的相关要求。**虚拟阶段页表测试基础设施的改进要求在配置两阶段测试环境时特别注意VS页表映射范围参数的准确性**。新增两阶段随机预取测试场景的环境配置和Scenario 7的高带宽测试环境配置要求。
