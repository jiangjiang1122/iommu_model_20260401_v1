...
## 更新摘要
**所做更改**   
- 新增128KB顺序写入两阶段翻译测试用例，增强性能测试框架的严格有序性保证验证
- 更新测试用例与负载章节，包含新的写操作测试场景
- 扩展架构总览图，反映写操作的完整数据流路径
- 完善故障排查指南，增加写操作相关的调试建议
...
**最新更新** 新增了针对128KB顺序写入操作的两阶段翻译测试用例，该测试通过严格的有序性保证验证，进一步完善了性能测试框架对写操作场景的覆盖能力。
...
**最新更新** 新增128KB顺序写入两阶段翻译测试用例，专门验证写操作的有序性保证和性能表现。
...
Perf-->>Top : "组装响应"
Top-->>App : "返回结果"
Note over App,Top : 新增128KB顺序写入测试验证写操作的严格有序性保证
```
...
- **新增** 128KB顺序写入两阶段：专门验证写操作的严格有序性保证，确保在高并发环境下写操作的顺序一致性。
- 多线程并发：通过线程化用例放大并发度，观察队列深度、吞吐与尾延迟变化。

**最新更新** 新增的128KB顺序写入两阶段测试用例（test_rp_seq128k_two_stage_write_thread.cc）专注于验证写操作在两级翻译过程中的严格有序性保证，这是性能测试框架的重要增强。
...
- [test_rp_seq128k_two_stage_write_thread.cc:1-200](file://rp/test_rp_seq128k_two_stage_write_thread.cc#L1-L200)
...
- **新增** 写操作有序性：对于顺序写入场景，需要特别关注写操作的严格有序性保证，避免乱序导致的性能退化。
...
- **新增** 写操作有序性问题：检查命令队列的重排序逻辑，确认写操作的顺序保证机制是否正常工作。
- 日志与脚本：使用提供的脚本提取IOPS、延迟分布与队列深度，辅助定位问题。

**最新更新** 针对新增的128KB顺序写入测试，增加了写操作有序性问题的排查建议，重点关注命令队列的重排序逻辑和顺序保证机制。
...
## 结论
场景9的高性能测试围绕IOMMU模型在大规模并发下的吞吐与延迟表现展开。通过清晰的层次化架构、可插拔的缓存策略与完善的性能采集体系，能够高效评估与优化系统行为。**最新更新** 新增的128KB顺序写入两阶段翻译测试用例进一步强化了对写操作有序性的验证能力，使性能测试框架更加完善。建议在真实负载下结合多线程用例与脚本工具，持续迭代参数与策略，以获得更稳健的性能结果。
...
- **新增** 写操作测试：test_rp_seq128k_two_stage_write_thread.cc专门用于验证128KB顺序写入的两阶段翻译性能与有序性保证。
