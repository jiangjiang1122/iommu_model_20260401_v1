...
## 更新摘要
**变更内容**
- **新增**：三关键两阶段翻译测试场景的综合性能基准验证 - Sequential 128K (S2-on) at 64GB/s with 256 streams, Sequential 128K (S2-on) at 128GB/s with 512 streams, Random 4K (S2-on) at 128GB/s with 512 streams
- **新增**：IOPS基准线建立，为未来优化提供参考基准
- **增强**：高带宽场景下的两阶段地址翻译性能验证能力
- **优化**：性能测试框架的扩展性，支持多种带宽配置和流数量组合
...
- **新增**：三关键两阶段翻译测试场景的综合性能基准验证，包括Sequential 128K (S2-on) at 64GB/s with 256 streams, Sequential 128K (S2-on) at 128GB/s with 512 streams, Random 4K (S2-on) at 128GB/s with 512 streams
- **新增**：IOPS基准线的建立，为未来优化提供可靠的参考标准
...
- **新增**：增强的PT Cache调度器性能测试框架（rp/test_rp_*_thread.cc），支持TEST_CFG_SKIP_PHASE1开关和新的Scenario 7测试场景
- **新增**：三关键两阶段翻译测试场景，涵盖不同带宽配置和访问模式
...
subgraph "增强的PT Cache调度器测试"
RP1["test_rp_seq128k_single_stage_thread.cc"]
RP2["test_rp_rand4k_single_stage_thread.cc"]
RP3["test_rp_seq128k_two_stage_thread.cc<br/>TEST_CFG_SKIP_PHASE1开关"]
RP4["test_rp_rand4k_two_stage_thread.cc"]
end
subgraph "三关键两阶段翻译测试场景"
S64["Scenario 64GB/s<br/>Sequential 128K<br/>256 streams<br/>S2-on"]
S128Seq["Scenario 128GB/s Seq<br/>Sequential 128K<br/>512 streams<br/>S2-on"]
S128Rand["Scenario 128GB/s Rand<br/>Random 4K<br/>512 streams<br/>S2-on"]
end
subgraph "预取测试框架"
...
- **新增**：三关键两阶段翻译测试场景，涵盖Sequential 128K (S2-on) at 64GB/s with 256 streams, Sequential 128K (S2-on) at 128GB/s with 512 streams, Random 4K (S2-on) at 128GB/s with 512 streams
...
### IOPS基准线建立与性能评估
**新增**：通过三关键两阶段翻译测试场景，建立了可靠的IOPS基准线，为未来的性能优化提供了明确的参考标准。

#### 基准线特征
- **Sequential 128K @ 64GB/s**: 建立中等带宽下的顺序访问基准
- **Sequential 128K @ 128GB/s**: 建立高带宽下的顺序访问基准  
- **Random 4K @ 128GB/s**: 建立高带宽下的随机访问基准
...
### Scenario 7高带宽测试场景
**新增**：Scenario 7测试用例专门针对两阶段地址翻译与S2-on配置下的高带宽场景进行了全面优化和验证。

#### 测试场景配置
- **测试类型**：TEST=rand4k_twostage_s2on_128g
- **访问模式**：随机4KB读取操作
- **带宽配置**：128GB/s高带宽环境
- **数据包数量**：10,000个数据包
- **页面分布**：1,250个唯一页面
...
#### 性能指标表现
- **IOPS性能**：76.29M IOPS，展现极高的并发处理能力
- **带宽利用率**：34.4GB/s，有效利用高带宽配置
- **PT命中率**：27.8%，反映页表缓存的效率水平
- **去重命中率**：88.2%，体现PT Cache去重机制的有效性
- **S2命中率**：99.8%，展示第二阶段地址翻译的极高缓存效率
...
class PerformanceMetrics {
+iops : 76.29M
+bandwidth_utilization : 34.4GB/s
+pt_hit_rate : 27.8%
+dedup_hit_rate : 88.2%
+s2_hit_rate : 99.8%
+validate_metrics()
}
Scenario7Test --> PerformanceMetrics : "生成性能指标"
```
...
**章节来源**
- [test_rp_rand4k_two_stage_thread.cc:14-370](file://rp/test_rp_rand4k_two_stage_thread.cc#L14-370)
...
- **新增**：Scenario 6性能表现：96.9%去重命中率、99.6% S2命中率、2.15ns PTW DDR延迟。
- **新增**：Scenario 7高带宽性能表现：76.29M IOPS、34.4GB/s带宽、99.8% S2命中率。
...
class Scenario6Performance {
+dedup_hit_rate : 96.9%
+s2_hit_rate : 99.6%
+ptw_ddr_latency : 2.15ns
}
class Scenario7Performance {
+iops : 76.29M
+bandwidth : 34.4GB/s
+pt_hit_rate : 27.8%
+dedup_hit_rate : 88.2%
...
Scenario6Performance --> PTCache : "性能验证"
Scenario7Performance --> PTCache : "高带宽验证"
```
...
- **新增**：三关键两阶段翻译测试场景与带宽配置的依赖关系：需要相应的带宽支持和S2-on配置。
...
Stats --> PTW
Stats --> Reorder
TestFramework["增强的测试框架<br/>TEST_CFG_SKIP_PHASE1"] --> Scheduler
...
- **新增**：Scenario 6性能突破：96.9%去重命中率、99.6% S2命中率、2.15ns PTW DDR延迟，展现了极高的缓存效率和地址翻译性能。
- **新增**：Scenario 7高带宽突破：76.29M IOPS、34.4GB/s带宽、99.8% S2命中率，验证了高带宽环境下两阶段地址翻译的高效性。
...
- **新增**：Scenario 6实现了96.9%去重命中率、99.6% S2命中率和2.15ns PTW DDR延迟的卓越性能表现。
- **新增**：Scenario 7实现了76.29M IOPS、34.4GB/s带宽和99.8% S2命中率的高带宽性能突破。
...
- **新增**：建议建立标准化的两阶段翻译测试流程，包括三关键场景的基准验证、UPDATE优先级验证、TEST_CFG_SKIP_PHASE1开关测试。
...
- 数据采集
  ...
  - **新增**：Scenario 6性能指标（去重命中率、S2命中率、PTW DDR延迟）
  - **新增**：Scenario 7性能指标（IOPS、带宽利用率、PT命中率、去重命中率、S2命中率）
...
- **新增**：Scenario 6性能指标解读：96.9%去重命中率表示PT Cache去重机制的高效性，99.6% S2命中率显示第二阶段地址翻译的极高缓存命中率，2.15ns PTW DDR延迟代表极低的页表遍历内存访问延迟
- **新增**：Scenario 7性能指标解读：76.29M IOPS体现高并发处理能力，34.4GB/s带宽展示高带宽利用率，99.8% S2命中率验证两阶段地址翻译的高效性
