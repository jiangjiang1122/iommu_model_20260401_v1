...
## 项目结构
IOMMU性能模型采用SystemC模块化设计，以“入口—解析—收集—缓存—查询—行走—转发/故障—重排序—出口”的多阶段流水线组织。关键模块与职责如下：
...
## 架构总览
IOMMU流水线以SystemC sc_fifo为数据通道，通过严格的阶段边界与背压控制实现高并行度与稳定性。关键特性：
- 阶段解耦：Parser/Collector/Cache/Walker/Forwarder/Fault各自独立，通过FIFO连接
...
- MSIPT Forwarder：根据MSI/MRIF标志路由至IMSIC或DMA路径
...
- 并行度优化
  ...
  - Parser/Collector/Cache/Walker/Forwarder/Fault阶段独立，最大化并行
- 背压与缓冲
...
## 结论
IOMMU流水线通过严格的阶段解耦、独立FIFO与轮询调度实现高并行度与稳定性。Parser/Collector/Cache/Walker/Forwarder/Fault各司其职，配合全局outstanding、AXI端口并发与FIFO可用空间形成多层背压。预取与批更新进一步提升吞吐，响应顺序与写保序策略兼顾一致性与性能。建议持续优化PTW响应顺序与Monitor机制，以提升整体稳定性与可观测性。
