...
- 处理流程
  ...
  - 当parser到达且DC/PC均完成，进入“收集”阶段（等待COLLECTOR_DELAY，设置TASK_COLLECTING）
- 路由决策与任务合并
...
**图表来源**
- [iommu_perf_collector.cc:277-452](file://iommu/iommu_perf_model/iommu_perf_collector.cc#L277-L452)
...
## 性能考量
- 处理延迟
  - COLLECTOR_DELAY：收集阶段固定延迟，保证流水线节奏
...
**章节来源**
- [iommu_perf_params.hh:116-140](file://iommu/iommu_perf_model/iommu_perf_params.hh#L116-L140)
- [iommu_perf_params.hh:149-160](file://iommu/iommu_perf_model/iommu_perf_params.hh#L149-L160)
...
## 结论
Collector模块在IOMMU性能模型中承担“结果聚合—路由决策—任务合并”的关键角色。通过两线程设计（收集与xDTW响应）、严格的outstanding控制与FIFO深度配置，以及完善的任务-缓存消息转换机制，Collector实现了高效、可扩展的翻译流水线。结合Walker Cache中间结果与PT Cache VA去重等优化，系统在高并发场景下仍能保持稳定吞吐与较低延迟。