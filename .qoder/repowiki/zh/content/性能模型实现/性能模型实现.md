...
## 更新摘要
**变更内容**
- 新增PT Cache去重刷新性能模型实现，支持占位CL到常规CL的转换和Buffer链表处理
- 增强PTW模块预取检测和批量响应处理功能，支持Burst读取优化
- 更新配置参数支持去重功能，包括PT_CACHE_DEDUP_ENABLED、PT_DEDUP_BUFFER_SIZE等
- 新增预取组监控线程，实现批量PT Cache更新和Dedup Buffer刷新
...
**更新** 新增PT Cache去重刷新性能模型实现，增强PTW模块预取检测和批量响应处理功能，更新配置参数支持去重功能，提供更精确的性能分析和调优能力。
...
- **新增** 去重与预取：PT Cache去重缓冲区管理，支持占位CL到常规CL的转换，预取组监控和批量更新。
...
**更新** 新增PT Cache去重刷新性能模型实现，增强PTW模块预取检测和批量响应处理功能，更新配置参数支持去重功能，提供更全面的性能分析能力。
...
**更新** 新增的PT Cache去重刷新性能模型、PTW模块预取检测和批量响应处理功能，显著增强了系统的性能优化能力和可观测性。新增的预取组监控线程和去重缓冲区管理提供了更精细的并发控制和资源管理。新增的稳态IOPS测量功能提供了更精确的性能基准，VA去重功能显著降低了PTW压力和DDR访问次数。建议在实际部署中结合具体工作负载调整FIFO深度、outstanding上限与缓存参数，充分利用VA去重、预取优化和稳态统计功能，以获得最佳性能。
