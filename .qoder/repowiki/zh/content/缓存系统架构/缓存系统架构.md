...
## 更新摘要
**变更内容**
- 更新PT Cache调度器章节，反映新的单线程调度器实现和任务级串行化机制
- 新增PT Cache统计收集能力增强说明，包括延迟直方图和任务跟踪改进
- 更新缓存替换保护机制，反映PT Cache的替换策略优化
- 新增PT Cache调度器详细架构分析
- 更新统计系统章节，反映PT Cache调度器的统计集成
...
**更新** 本次更新重点关注PT Cache系统架构的重大改进，包括新的单线程调度器实现、增强的统计收集能力和替换保护机制优化。
...
**更新** PT Cache经过重大架构改进，实现了新的单线程调度器和增强的统计收集能力。
...
- **新增** 单线程调度器：合并pt_worker_thread和pt_update_worker_thread，实现任务级串行化，REQUEST优先处理，UPDATE次之。
- **新增** 增强统计收集：支持FIFO等待时间统计、任务跟踪、延迟直方图和相位时间戳记录。
...
class PTSchedulerThread {
+pt_scheduler_thread() void
+execute_pt_request(req) CacheMessage
+execute_pt_update(req) void
+record_task_completion(cache_name, start_time, end_time) void
}
PTCache --> PTSchedulerThread : "单线程调度器"
```

**图表来源**
- [cache_base.h:26-256](file://iommu/cache_src/cache/cache_base.h#L26-L256)
- [pt_cache.h:8-54](file://iommu/cache_src/cache/pt_cache.h#L8-L54)
- [cache_subsystem.cpp:311-339](file://iommu/cache_src/subsystem/cache_subsystem.cpp#L311-L339)
...
**新增** PT Cache实现了全新的单线程调度器，替代了原有的双线程架构：

- **任务级串行化**：合并pt_worker_thread和pt_update_worker_thread，确保同一时间只有一个任务在执行
- **优先级调度**：REQUEST任务优先级高于UPDATE任务，避免更新操作阻塞查询请求
- **FIFO等待时间统计**：精确记录任务在FIFO中的等待时间，用于性能分析
- **任务跟踪集成**：支持begin/end事件跟踪，提供完整的任务生命周期监控

```mermaid
flowchart TD
Start(["PT Cache调度器启动"]) --> CheckFIFO{"检查pt_request_fifo"}
CheckFIFO --> |有REQUEST| ProcessRequest["处理REQUEST任务"]
CheckFIFO --> |无REQUEST| CheckUpdate{"检查pt_update_fifo"}
CheckUpdate --> |有UPDATE| ProcessUpdate["处理UPDATE任务"]
CheckUpdate --> |无任务| Wait["等待下一个任务"]
ProcessRequest --> FIFOWait["计算FIFO等待时间"]
FIFOWait --> ExecuteLookup["执行execute_pt_request"]
ExecuteLookup --> RecordTask["记录任务完成"]
RecordTask --> PushResponse["推送响应到pt_hit_response_fifo"]
PushResponse --> CheckFIFO
ProcessUpdate --> ExecuteUpdate["执行execute_pt_update"]
ExecuteUpdate --> CheckFIFO
Wait --> CheckFIFO
```
...
**新增** PT Cache调度器集成了增强的统计收集系统：

- **FIFO等待时间**：`stats_.accumulate_phase_task_wait("pt_cache", 0, fifo_wait_ns)` 记录任务排队等待时间
- **相位时间戳**：`stats_.record_pt_phase_timestamp()` 记录查询阶段的开始和结束时间
- **任务跟踪**：`trace_task_event()` 提供任务生命周期的详细跟踪
- **延迟直方图**：支持PT Cache特定的延迟分布统计
...
- **更新** PT Cache采用新的单线程调度器，实现任务级串行化和优先级调度。
...
- **新增** PT Cache性能优化
  - 单线程调度器实现了任务级串行化，避免了双线程竞争带来的性能损失。
  - REQUEST优先级确保查询请求不会被更新操作阻塞，提升了响应延迟性能。
  - 增强的统计收集能力提供了更精确的性能分析数据。
  - FIFO等待时间统计帮助识别潜在的队列拥塞问题。
- **新增** WalkerCache性能优化
...
- 统计缺失或异常
  ...
  - **新增** 检查PT Cache调度器的统计输出，确认FIFO等待时间和任务跟踪是否正常。
- **新增** PT Cache相关问题
  - 检查单线程调度器是否正常工作，确认REQUEST和UPDATE任务的优先级设置。
  - 分析FIFO等待时间统计，识别潜在的队列拥塞。
  - 验证任务跟踪功能，检查begin/end事件的完整性。
- **新增** WalkerCache相关问题
...
**更新** 本次PT Cache系统架构的重大改进进一步提升了缓存系统的性能和可靠性，包括单线程调度器实现、增强的统计收集能力和替换保护机制优化。这些改进为复杂的页表缓存场景提供了更好的支持，特别是在任务级串行化和性能统计方面的提升，为系统性能优化提供了更精确的数据支撑。
...
**新增** 基于测试报告的PT Cache性能分析：

从`PT_CACHE_ACCESS_ANALYSIS_50TASKS_20260609.md`可以看出：
- **单线程调度器效果**：50次任务中，REQUEST任务平均延迟显著低于UPDATE任务
- **FIFO等待时间**：平均等待时间为150ns，峰值达到800ns，主要集中在高负载时段
- **任务跟踪**：begin/end事件完整记录，支持任务生命周期分析
- **统计集成**：相位时间戳记录精确到微秒级别

**章节来源**
- [PT_CACHE_ACCESS_ANALYSIS_50TASKS_20260609.md:1-133](file://PT_CACHE_ACCESS_ANALYSIS_50TASKS_20260609.md#L1-L133)
...
- **PT Cache集成**：新增PT Cache特定的统计收集能力，包括FIFO等待时间和任务跟踪
