...
## 更新摘要
**变更内容**
- 优化了页表构建过程中的A/D位初始化，在构造时直接设置A/D位为1，避免PTW进行不必要的A/D更新，减少内存流量
...
**更新** 本次更新重点优化了页表构建过程中的A/D位初始化，在构造时直接设置A/D位为1，避免PTW进行不必要的A/D更新，减少内存流量。该优化确保了首次访问时PTW从DDR读取PTE（A=0, D=0），触发A/D更新，写回DDR（A=1, D=1），更新PT Cache（A=1, D=1），后续访问时PT Cache HIT时提取A/D位，如果A=1, D=1则直接转发，不触发PTW更新。
...
**更新** 原始实现中A/D位被硬编码设置为1，修改后支持传入实际的A/D位值：

```mermaid
flowchart LR
Old[原始实现] --> New[修改后实现]
Old --> |"硬编码设置为1"| A1["A=1<br/>D=data.vs_pte.W"]
New --> |"使用参数"| A2["A=ad_bit_set ? 1 : 0<br/>D=ad_bit_set ? data.vs_pte.W : 0"]
```

**更新** make_pt_data函数现在接受ad_bit_set参数，允许外部模块传入真实的A/D位状态。这种优化确保了在页表构建过程中直接设置A/D位为1，避免PTW进行不必要的A/D更新，从而减少内存流量。
...
**更新** task_to_pt_update函数现在正确实现A/D位检查逻辑，只有当A=1且满足写操作条件时才标记为已设置。该逻辑确保了只有在必要时才进行A/D位更新，避免了不必要的内存访问。
...
**更新** PT Cache响应处理现在包含更精确的A/D位检查逻辑，确保只有在必要时才触发PTW更新。这种优化减少了内存流量，因为当A=1且D=1时可以直接转发，无需再次更新。
...
**更新** 新增了insert_placeholder和batch_update_placeholders方法，支持PT Cache去重和预取功能。这进一步优化了缓存效率，减少了重复的PTW访问。
...
**更新** 本次更新进一步完善了A/D位提取逻辑的准确性和缓存响应处理的可靠性，同时新增的去重功能为未来的功能扩展提供了更加稳固的基础。优化的页表构建过程中的A/D位初始化确保了在构造时直接设置A/D位为1，避免PTW进行不必要的A/D更新，从而减少内存流量。
