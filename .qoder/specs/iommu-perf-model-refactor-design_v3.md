...
**v3变更摘要**（参照架构文档 IOMMU Arch Spec 0.31 第4.5章节"Walker Cache地址翻译中间值缓存"）：
- **Walker Cache替换策略修正**：SRRIP → pLRU（与架构文档对齐）
- **Walker Cache详细设计重写**：补充Cache line结构（{valid, gscid, pscid, VA字段, PPN}）、Hash索引函数（Hash_1/Hash_2/Hash_3）、并行7-way查询机制、命中优先级（PTWc_3 > PTWc_2 > PTWc_1）
- **PTWc_1模式激活条件**：Sv39模式下PTWc_1禁用（仅3级walk无VPN[3]），仅Sv48/Sv57时激活
- **叶PTE Walker Cache更新错误修正**：移除ptw_rsp_thread中叶PTE对Walker Cache的update调用（叶PTE最终翻译结果属于IOTLB/PT Cache，不属于Walker Cache中间值缓存）
- **非叶PTE更新细化**：明确各级别非叶PTE更新到PTWc_1/PTWc_2/PTWc_3的映射关系
- **Walker Cache失效联动**：补充DC Cache、PC Cache、PT Cache失效操作对Walker Cache的联动失效触发
- **实现差距记录**：记录当前PTW实现（iommu_perf_ptw.cc）缺失的Walker Cache功能项
...
### 3.4 Walker Cache `walker_cache_t`（三级中间值缓存）（v3重写，参照架构文档4.5）

Walker Cache用于缓存page table walk**一阶段地址翻译过程中**VA/GPA到**下一级页表物理地址PPN**的中间结果。
注意：Walker Cache仅缓存非叶PTE的中间PPN（指向下一级页表的物理地址），**不缓存叶PTE的最终翻译结果**（那属于IOTLB/PT Cache）。

#### 3.4.1 VA地址字段划分与缓存表规划

以Sv48为例，VA地址按VPN分为4段：VPN[3](9bit) / VPN[2](9bit) / VPN[1](9bit) / VPN[0](9bit) + page_offset(12bit)。
Sv39时VA分为3段：VPN[2](9bit) / VPN[1](9bit) / VPN[0](9bit) + page_offset(12bit)，无VPN[3]。

根据VA地址字段划分，规划3部分缓存表：

| 缓存表 | 映射方式 | 表数量 | 每表大小 | 缓存内容 | 对应Walk级别 | 备注 |
|--------|----------|--------|----------|----------|-------------|------|
| PTWc_1 | 直接映射 | 1 | s=64 | VPN最高段→下级PT PPN | level=LEVELS-1 (Sv48: level=3) | Sv39时禁用（无VPN[3]） |
| PTWc_2 | 2路组相联 | 2 | s=64 | VPN[3:2]→下级PT PPN | level=LEVELS-2 (Sv48: level=2) | pLRU替换 |
| PTWc_3 | 4路组相联 | 4 | s=64 | VPN[3:1]→下级PT PPN | level=LEVELS-3 (Sv48: level=1) | pLRU替换 |

总计7个缓存表（1+2+4），同时查询，类同于7-way组相联。s暂定=64。

#### 3.4.2 Cache Line结构

每个Cache Line包含以下字段：
```
struct walker_cache_line_t {
    uint8_t  valid;        // 有效位
    uint32_t gscid;        // Guest Soft-Context ID（隔离不同VM）
    uint32_t pscid;        // Process Soft-Context ID（隔离不同进程）
    uint64_t va_tag;       // VA地址标签（VPN切分字段，具体宽度取决于PTWc_1/2/3）
    uint64_t ppn;          // 地址转换中间结果：下一级页表的物理页号PPN
};
```

各表的va_tag含义：
- **PTWc_1**: va_tag = VPN[3]（Sv48时9bit；Sv57时可扩展）
- **PTWc_2**: va_tag = {VPN[3], VPN[2]}（Sv48时18bit）
- **PTWc_3**: va_tag = {VPN[3], VPN[2], VPN[1]}（Sv48时27bit）

Sv39模式下：PTWc_1禁用，PTWc_2的va_tag = VPN[2]（9bit），PTWc_3的va_tag = {VPN[2], VPN[1]}（18bit）。

#### 3.4.3 Hash索引函数

三个Hash函数用于计算各缓存表的index（取LSB散列）：

```
// Hash_1: PTWc_1索引（直接映射，1个表）
// 输入：gscid, pscid, VPN[LEVELS-1]
hash_1_index = (gscid ^ pscid ^ VPN[LEVELS-1]) % s;

// Hash_2: PTWc_2索引（2路组相联，2个表共享同一index）
// 输入：gscid, pscid, VPN[LEVELS-1], VPN[LEVELS-2]
hash_2_index = (gscid ^ pscid ^ VPN[LEVELS-1] ^ VPN[LEVELS-2]) % s;

// Hash_3: PTWc_3索引（4路组相联，4个表共享同一index）
// 输入：gscid, pscid, VPN[LEVELS-1], VPN[LEVELS-2], VPN[LEVELS-3]
hash_3_index = (gscid ^ pscid ^ VPN[LEVELS-1] ^ VPN[LEVELS-2] ^ VPN[LEVELS-3]) % s;
```

注：s=64时，index范围为[0,63]。

#### 3.4.4 并行查询与命中优先级

查询时同时计算3个Hash函数、查询3部分缓存的7个Cache line，按命中优先级输出：

```
PTWc_3 > PTWc_2 > PTWc_1
```

- PTWc_3命中：跳过LEVELS-1, LEVELS-2, LEVELS-3三个级别，从LEVELS-4级开始walk
- PTWc_2命中：跳过LEVELS-1, LEVELS-2两个级别，从LEVELS-3级开始walk
- PTWc_1命中：跳过LEVELS-1一个级别，从LEVELS-2级开始walk
- 全部未命中：从根级LEVELS-1开始正常walk

lookup伪代码：
```
int walker_cache_t::lookup(uint32_t gscid, uint32_t pscid,
                            uint64_t iova, uint8_t mode) {
    uint16_t vpn[5];
    uint8_t LEVELS;
    extract_vpn_for_cache(iova, mode, vpn, &LEVELS);

    // 1. 查PTWc_3（4路，优先级最高）
    if (LEVELS >= 4) {  // Sv48+才有PTWc_3（需要4级walk才有3级中间结果）
        uint32_t idx3 = hash_3(gscid, pscid, vpn, LEVELS);
        for (int way = 0; way < 4; way++) {
            walker_cache_line_t& line = ptwc_3[way][idx3];
            if (line.valid && line.gscid == gscid && line.pscid == pscid &&
                line.va_tag == make_va_tag_3(vpn, LEVELS)) {
                last_hit_ppn = line.ppn;
                ptwc_3_plru_update(idx3, way);  // 更新pLRU状态
                return LEVELS - 3;  // 从level LEVELS-4开始walk
            }
        }
    }

    // 2. 查PTWc_2（2路）
    if (LEVELS >= 3) {  // Sv39+才有PTWc_2
        uint32_t idx2 = hash_2(gscid, pscid, vpn, LEVELS);
        for (int way = 0; way < 2; way++) {
            walker_cache_line_t& line = ptwc_2[way][idx2];
            if (line.valid && line.gscid == gscid && line.pscid == pscid &&
                line.va_tag == make_va_tag_2(vpn, LEVELS)) {
                last_hit_ppn = line.ppn;
                ptwc_2_plru_update(idx2, way);
                return LEVELS - 2;  // 从level LEVELS-3开始walk
            }
        }
    }

    // 3. 查PTWc_1（直接映射）
    if (LEVELS >= 4) {  // PTWc_1仅Sv48+激活（Sv39时无VPN[3]，禁用）
        uint32_t idx1 = hash_1(gscid, pscid, vpn, LEVELS);
        walker_cache_line_t& line = ptwc_1[idx1];
        if (line.valid && line.gscid == gscid && line.pscid == pscid &&
            line.va_tag == make_va_tag_1(vpn, LEVELS)) {
            last_hit_ppn = line.ppn;
            return LEVELS - 1;  // 从level LEVELS-2开始walk
        }
    }
...
当ptw_rsp_thread解析到**非叶PTE**时，将中间PPN更新到对应级别的Walker Cache表。

更新映射关系（以Sv48为例）：
- 非叶PTE at level=3 → 更新PTWc_1（VPN[3] → PPN）
- 非叶PTE at level=2 → 更新PTWc_2（{VPN[3],VPN[2]} → PPN）
- 非叶PTE at level=1 → 更新PTWc_3（{VPN[3],VPN[2],VPN[1]} → PPN）
- level=0 的PTE必为叶PTE → 不更新Walker Cache（结果存入IOTLB/PT Cache）

**注意**：叶PTE（R==1或X==1）的最终翻译PPN不写入Walker Cache。叶PTE结果由PT Cache（IOTLB）缓存。Walker Cache仅存储**中间值**（非叶PTE指向下一级页表的PPN）。

update伪代码：
```
void walker_cache_t::update(uint32_t gscid, uint32_t pscid,
                             uint64_t iova, uint8_t mode,
                             int8_t level, uint64_t ppn) {
    uint16_t vpn[5];
    uint8_t LEVELS;
    extract_vpn_for_cache(iova, mode, vpn, &LEVELS);

    int cache_level = LEVELS - 1 - level;  // level到缓存表的映射

    if (cache_level == 0 && LEVELS >= 4) {
        // 更新PTWc_1（直接映射）
        uint32_t idx = hash_1(gscid, pscid, vpn, LEVELS);
        ptwc_1[idx] = {1, gscid, pscid, make_va_tag_1(vpn, LEVELS), ppn};
    }
    else if (cache_level == 1 && LEVELS >= 3) {
        // 更新PTWc_2（2路组相联，pLRU选择替换way）
        uint32_t idx = hash_2(gscid, pscid, vpn, LEVELS);
        int victim_way = ptwc_2_plru_victim(idx);
        ptwc_2[victim_way][idx] = {1, gscid, pscid, make_va_tag_2(vpn, LEVELS), ppn};
        ptwc_2_plru_update(idx, victim_way);
    }
    else if (cache_level == 2) {
        // 更新PTWc_3（4路组相联，pLRU选择替换way）
        uint32_t idx = hash_3(gscid, pscid, vpn, LEVELS);
        int victim_way = ptwc_3_plru_victim(idx);
        ptwc_3[victim_way][idx] = {1, gscid, pscid, make_va_tag_3(vpn, LEVELS), ppn};
        ptwc_3_plru_update(idx, victim_way);
    }
    // cache_level >= 3: 理论上不会到达（最多3级中间结果）
}
```

#### 3.4.6 替换策略：pLRU（伪最近最少使用）

**v3修正**：v2使用SRRIP替换策略，与架构文档4.5不一致。v3改为pLRU（pseudo-LRU），与架构文档对齐。

- **PTWc_1**：直接映射，无需替换策略（直接覆盖）
- **PTWc_2**：2路组相联，1-bit pLRU per set（指向最近未使用的way）
- **PTWc_3**：4路组相联，3-bit tree-based pLRU per set

```
// PTWc_2 pLRU: 1 bit per set
uint8_t ptwc_2_plru[64];   // ptwc_2_plru[set] = 0→替换way0, 1→替换way1

// PTWc_3 pLRU: 3 bits per set (binary tree)
//        bit[2]
//       /      \
//    bit[1]   bit[0]
//    / \      / \
// way0 way1 way2 way3
uint8_t ptwc_3_plru[64];   // 3-bit tree per set

int ptwc_3_plru_victim(uint32_t set) {
    uint8_t tree = ptwc_3_plru[set];
    if ((tree >> 2) & 1) {
        // 右子树：选way2或way3
        return (tree & 1) ? 3 : 2;
    } else {
...
Walker Cache的失效由以下操作联动触发：
- **DC Cache失效**（IODIR.INVAL_DDT）：按gscid失效Walker Cache中匹配的所有条目
- **PC Cache失效**（IODIR.INVAL_PDT）：按gscid+pscid失效Walker Cache中匹配的条目
- **PT Cache失效**（IOTINVAL.VMA/GVMA/ALL）：根据失效范围（gscid/pscid/iova/全部）失效Walker Cache
- **全局失效**（IOTINVAL.ALL）：清除Walker Cache所有条目

```
void walker_cache_t::invalidate_by_gscid(uint32_t gscid);        // DC失效联动
void walker_cache_t::invalidate_by_gscid_pscid(uint32_t gscid, uint32_t pscid);  // PC失效联动
void walker_cache_t::invalidate_by_gscid_pscid_iova(uint32_t gscid, uint32_t pscid, uint64_t iova); // PT失效联动
void walker_cache_t::invalidate_all();                             // 全局失效
```

#### 3.4.8 Walker Cache数据结构总体定义

```
walker_cache_t
├── walker_cache_line_t ptwc_1[64]       -- 直接映射，缓存VPN最高段 → 下级PT PPN
├── walker_cache_line_t ptwc_2[2][64]    -- 2路组相联，缓存VPN[LEVELS-1:LEVELS-2] → 下级PT PPN
├── walker_cache_line_t ptwc_3[4][64]    -- 4路组相联，缓存VPN[LEVELS-1:LEVELS-3] → 下级PT PPN
├── uint8_t ptwc_2_plru[64]             -- PTWc_2 per-set pLRU bit（1-bit）
├── uint8_t ptwc_3_plru[64]             -- PTWc_3 per-set pLRU tree（3-bit）
├── uint64_t last_hit_ppn               -- 最近一次命中的PPN（供get_ppn()返回）
├── lookup(gscid,pscid,iova,mode) → hit_level(-1=miss, >=0=命中级别)
├── get_ppn() → last_hit_ppn            -- 获取最近命中的PPN
├── update(gscid,pscid,iova,mode,level,ppn) -- 仅非叶PTE调用
├── invalidate_by_gscid(gscid)
├── invalidate_by_gscid_pscid(gscid,pscid)
├── invalidate_by_gscid_pscid_iova(gscid,pscid,iova)
├── invalidate_all()
└── pLRU替换（v3修正：v2为SRRIP，与架构文档不一致）
```
...
| Walker Cache | `sc_mutex walker_cache_mtx` | PTW线程 + Fault/CQ线程 | 并发查找/更新/失效（v3：lookup+update在PTW线程，invalidate在Fault/CQ线程） |
...
| 文件 | 行数估算 | 内容 |
|------|----------|------|
| `iommu/iommu_task.hh` | ~400 | iommu_task_t, task_state_t, walk_context_t, ddr_req_entry_t, ddr_rsp_entry_t, ddr_pending_entry_t, ctrl_path_ddr_req_t, walker_cache_line_t, walker_cache_t（v3：含完整Cache line结构、pLRU状态、Hash索引、并行7-way查询、失效联动接口） |
...
| `iommu/iommu_perf_ptw.cc` | ~700 | PTW的2个线程（v2：req/rsp解耦+嵌套walk状态机）+ Walker Cache集成（v3：req中lookup、rsp中仅非叶PTE update、pLRU替换） |
...
### Phase 1：基础数据结构
1. 创建`iommu/iommu_task.hh` -- 定义iommu_task_t、task_state_t、walk_context_t（v2增强：walk_phase/vs_level/gs_level等）、ddr_req_entry_t、ddr_rsp_entry_t、ddr_pending_entry_t、ctrl_path_ddr_req_t、walker_cache_line_t、walker_cache_t（v3重写：pLRU替换、Hash索引、并行7-way查询、失效联动）
...
14. `iommu_perf_ptw.cc` -- PTW的2个线程（v2：req/rsp解耦+嵌套walk状态机）+ Walker Cache集成（v3：req中并行7-way lookup、rsp中仅非叶PTE update）
...
void iommu_top::ptw_req_thread() {
    while (true) {
        ...
        // ========== 2. VS-stage Walk初始化 ==========
        {
            ...
            // Walker Cache查询（v3细化：并行查PTWc_3/PTWc_2/PTWc_1，按优先级返回）
            // 查询详细逻辑见Section 3.4.4
            // 命中时跳过前序层级，减少DDR访问次数
            // 例如Sv48下PTWc_3命中：从level=0开始walk（跳过level 3,2,1）
            walker_cache_mtx.lock();
            int hit_level = walker_cache.lookup(task->GSCID, task->PSCID,
                                                 task->iova, task->iosatp.MODE);
            if (hit_level >= 0) {
                a = walker_cache.get_ppn() * PAGESIZE;  // v3: 使用get_ppn()获取last_hit_ppn
                i = hit_level - 1;  // 从命中级别的下一级开始walk
            }
    ...
    while (true) {
        ...
        case PTW_VS_WALK: {
            ...
            if (pte.R == 1 || pte.X == 1) {
                ...
                // v3修正：移除叶PTE的Walker Cache更新
                // 叶PTE的最终翻译PPN属于IOTLB（PT Cache），不属于Walker Cache中间值缓存
                // Walker Cache仅缓存非叶PTE的中间PPN（见Section 3.4.5）
                // 原v2代码（已删除）：
                // walker_cache_mtx.lock();
                ...
                // Walker Cache更新（非叶PTE中间PPN → 对应级别缓存表）
                // 非叶PTE的PPN指向下一级页表，属于Walker Cache的缓存目标
                // update()内部根据当前level自动映射到PTWc_1/PTWc_2/PTWc_3
                // 例如Sv48: level=3→PTWc_1, level=2→PTWc_2, level=1→PTWc_3
                walker_cache_mtx.lock();
                walker_cache.update(task->GSCID, task->PSCID, task->iova,
                                     task->iosatp.MODE, task->walk_ctx.level, pte.PPN);
                walker_cache_mtx.unlock();
...
**Walker Cache交互**（v3更新，参照架构文档4.5）：
- **查询**：ptw_req_thread中walk开始前查Walker Cache（并行查PTWc_3/PTWc_2/PTWc_1，按优先级PTWc_3>PTWc_2>PTWc_1输出），命中则跳过前序层级，从命中级别的下一级开始walk
- **更新**：ptw_rsp_thread中**仅在非叶PTE**时更新对应级别的Walker Cache条目（level→PTWc表映射见Section 3.4.5）
- **v3修正**：v2中叶PTE也调用了walker_cache.update()，这是错误的。叶PTE的最终翻译PPN属于IOTLB（PT Cache），不属于Walker Cache中间值缓存。v3已移除叶PTE的update调用
- **三级结构**：PTWc_1（直接映射，最高级PPN，Sv39禁用）、PTWc_2（2路pLRU，中间级）、PTWc_3（4路pLRU，最低级）
- **失效联动**：DC/PC/PT Cache失效时联动触发Walker Cache对应条目失效（见Section 3.4.7）
...
### 13.9 Walker Cache实现差距记录（v3新增）

基于对当前PTW实现代码（`iommu_perf_ptw.cc`，655行）的完整审查，记录Walker Cache功能与SPEC设计之间的实现差距。

**当前实现状态**：

| 项目 | SPEC要求 | 当前实现 | 差距 |
|------|----------|----------|------|
| `walker_cache_t` 类定义 | Step 1要求在`iommu_task.hh`中定义 | **未定义** | 需新增完整类定义（含Cache line结构、pLRU状态、lookup/update/invalidate方法） |
| `walker_cache_t` 实例 | Step 6要求在`iommu_top.hh`中声明 | **未声明**（仅有`walker_cache_mtx`） | 需在`iommu_top.hh`中新增`walker_cache_t walker_cache;`成员 |
| `ptw_req_thread` lookup | Section 12.13要求walk开始前查Walker Cache | **完全缺失** | 需在VPN提取后、DDR请求前插入`walker_cache.lookup()`调用 |
| `ptw_rsp_thread` 非叶PTE update | Section 12.14要求非叶PTE解析后更新Walker Cache | **完全缺失** | 需在非叶PTE分支中插入`walker_cache.update()`调用 |
| `ptw_rsp_thread` 叶PTE | v2错误地包含叶PTE update | **N/A（未实现）** | v3已修正：叶PTE不应更新Walker Cache |
| 失效联动 | Section 3.4.7要求DC/PC/PT失效联动 | **未实现** | 需在`fault_cq_proc_thread`中IOTINVAL命令处理时调用`walker_cache.invalidate_*()` |

**修复优先级**：
1. **P0**：在`iommu_task.hh`中定义`walker_cache_t`类（含Section 3.4.8中的完整数据结构）
2. **P0**：在`iommu_top.hh`中声明`walker_cache_t walker_cache`实例
3. **P0**：在`ptw_req_thread`中添加Walker Cache lookup（在VPN提取和canonical check之后，DDR请求发送之前）
4. **P0**：在`ptw_rsp_thread`的PTW_VS_WALK非叶分支中添加Walker Cache update
5. **P1**：在`fault_cq_proc_thread`的IOTINVAL命令处理中添加Walker Cache失效联动
6. **P2**：添加Walker Cache统计计数器（hit/miss/update/invalidate次数）
...
### Step 1: 创建 `iommu/iommu_task.hh`（新文件，~400行）（v3：行数增加，含walker_cache_t详细定义）
...
- `struct walker_cache_line_t` — Walker Cache单条缓存行（v3新增，见Section 3.4.2）
- `class walker_cache_t` — Walker Cache三级中间值缓存（v3重写，参照架构文档4.5）：
  - 数据成员：`ptwc_1[64]`, `ptwc_2[2][64]`, `ptwc_3[4][64]`, `ptwc_2_plru[64]`, `ptwc_3_plru[64]`, `last_hit_ppn`
  - `lookup(gscid,pscid,iova,mode)` → 并行查PTWc_3/PTWc_2/PTWc_1，按优先级返回hit_level（见Section 3.4.4）
  - `get_ppn()` → 返回最近命中的PPN
  - `update(gscid,pscid,iova,mode,level,ppn)` → **仅非叶PTE调用**，根据level映射到PTWc_1/2/3（见Section 3.4.5）
  - `invalidate_by_gscid/gscid_pscid/gscid_pscid_iova/all()` → 失效接口（见Section 3.4.7）
  - 替换策略：**pLRU**（v3修正，v2为SRRIP，与架构文档不一致）
  - PTWc_1激活条件：仅LEVELS>=4（Sv48+）时激活，Sv39时禁用
...
- `walker_cache_t walker_cache` 实例（v3注意：当前实现中**缺失此声明**，仅有`walker_cache_mtx`，需补充）
- Walker Cache统计计数器（v3新增）：`stat_walker_cache_hits`, `stat_walker_cache_misses`
